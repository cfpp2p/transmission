...
tr_torrentCheckSeedLimit( tr_torrent * tor )
{
    ...
    /* if tor->isStopping has been set don't check seed limit again 11/26/12 */
...
onTrackerResponse( tr_torrent * tor, const tr_tracker_event * event, void * unused UNUSED )
{
    switch( event->messageType )
    {
	    ...
	    /*if( allAreSeeds && event->pexCount < 1 || allAreSeeds &&event->pexCount > 1 ) 
	    ...
	    if( allAreSeeds && event->pexCount == 1 ) 
	        tr_tordbg( tor, "Got %zu seed from tracker", event->pexCount );

            if( !allAreSeeds && event->pexCount == 1 )
                tr_tordbg( tor, "Got %zu peer from tracker", event->pexCount );*/

	    if( allAreSeeds )
		tr_tordbg( tor, "Number of seeds received from tracker: %zu", event->pexCount );
	    else
		tr_tordbg( tor, "Number of peers received from tracker: %zu", event->pexCount );		    
...
setLocalErrorIfFilesDisappeared( tr_torrent * tor )
{
    ...
    if( disappeared )
    {
        ...
        tr_torrentSetLocalError( tor, "%s", _( "WARNING! possible user error! files disappeared!!! to IGNORE: after verify use Start/Resume Now" ) );
    }
    ...
    if(( dir = opendir( path )))
    {
        ...
        while(( d = readdir( dir )))
        {
            ...
            if( name && strcmp( name, "." ) && strcmp( name, ".." ) )
                tr_list_append( &files, tr_buildPath( name, NULL ) );
        }
...
usePieceTemp( tr_torrent * tor, tr_file_index_t i )
{
  ...
  if( tor->info.files[i].dnd )
  {
    ...
    if( tr_torrentFindFile2( tor, i, NULL, NULL ) )
    {
      ...
      if( ((int)i + 1) < tor->info.fileCount && !tr_torrentFindFile2( tor, ((int)i + 1), NULL, NULL ) )
        tr_moveFile( oldpathPTlast, newpathPTlast, &renamed );
      ...
      tr_free( oldpath );
      tr_free( sub );
      tr_free( newpathPTfirst );
      tr_free( newpathPTlast );
      tr_free( oldpathPTfirst );
...
//    char * oldpath = NULL;
	...
	char * oldpath = tr_buildPath( home, ".Trash", sub, NULL );
	  ...
	  tr_tordbg( tor, "rc1b:Unable to restore file:%d.", (int)i );
...
setFileDND( tr_torrent * tor, tr_file_index_t file_index, int8_t dnd )
{
    ...
    if( !file->dnd && tr_cpFileIsComplete( &tor->completion, file_index ) )
    {
    ...
    file->dnd = dnd;
    /* we need usePieceTemp to always check for restorable data. */
    ...
    if( fpmovept || lpmovept )
    {
      ...
      if( fpmovept )
      {
	if( !tr_torrentFindFile2( tor, file_index, NULL, NULL ) )
    ...
    if( fpindex == lpindex )
    {
        ...
        /* Handle removal in usePieceTemp */
    }
    else
    {
        tr_piece_index_t p;
