...
getOldCacheDir( void )
{
    ...
    if( !path )
    {
        ...
        path = tr_buildPath( getHomeDir( ), "Library", "Caches", "TurboTransmission", NULL );
#else
        path = tr_buildPath( getOldConfigDir( ), "cache", NULL );
#endif
    }
...
{
    static char * s = NULL;
	appname = "TurboTransmission";  /* included 3/18/12 */ 

    if( !appname || !*appname )
        appname = "TurboTransmission";
...
tr_getDefaultDownloadDir( void )
{
    ...
    if( user_dir == NULL )
    {
        ...
        if( content && content_len>0 )
        {
		    ...
		    else if( !strcmp (value, "$HOME") ) /* r13690 */
