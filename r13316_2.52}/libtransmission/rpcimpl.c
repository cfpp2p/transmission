...
/* #include <stdio.h>  for blocklist ch - not working */
    ...
    else /* successfully fetched the blocklist... */
    {
        ...
        if( !errno )
        {
            ...
            if( fd < 0 )
                ...
                gzFile gzf = gzopen( filename, "rb" ); /* Adding "b" no help for bt */
                /* http://www.gzip.org/fixgz.c -- remove '\r' */

                if( gzf ) {
                    const size_t buflen = ( 512 * 256 ); /* 128 KiB buffer-1024*128 */
            ...
            struct tr_rpc_idle_data  * idle_data UNUSED )
{
    ...
    if( tr_bencDictFindStr( args_in, TR_PREFS_KEY_TRASH_TEMP_DIR, &str ) )
