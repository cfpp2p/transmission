 ...
 *
 * $Id: peer-mgr.c 11881 2011-02-12 17:27:47Z jordan $
 */
...
typedef struct tr_torrent_peers
{
    ...
    /* Before the endgame this should be 0. In endgame, it >0 is contains
     * the average number of pending requests per peer. Only peers which have
     * more pending requests are considered 'fast' are allowed to request a
     * block that's already been requested from another (slower?) peer. */
    int                        endgame;
}
Torrent;
...
#define bl_tordbg( t, ... ) \
    do { \
        if( tr_msgLoggingIsActive( TR_MSG_DBG ) ) \
            tr_msg( __FILE__, __LINE__, TR_MSG_DBG, tr_torrentName( t->tor ), __VA_ARGS__ ); \
    } while( 0 )
...
updateEndgame( Torrent * t )
{
    ...
    else if( !t->endgame ) /* only calculate if we're not in endgame */ 
    {
...
{
    /*  SEE LINE 1299: bl_tordbg( t, "IP:%s bad piece strike count is %d",
	...
	bl_tordbg( t, "IP:%s banned. 2 bad pieces sent!", tr_atomAddrStr( atom ) );    
    }
}
                   ...
                   void          * vmanager )
{
    ...
    else /* looking good */
    {
	    ...
	    bl_tordbg( t, "IP:%s refused reconnection (bad pieces)", tr_atomAddrStr( atom ) );
        }
                       ...
                       int          socket )
{
    ...
    {
	tr_dbg( "Incoming blocklisted IP %s dropped", tr_address_to_string( addr ) );
                  ...
                  const tr_pex * pex, int8_t seedProbability )
{
	...
	{
	    tr_tordbg( tor, "Blocklisted IP:%s dropped as pex peer", tr_address_to_string( &pex->addr ) );
        }
                    ...
                    int              success )
{
    if( !success )
    {
        ...
        for( i = 0; i < peerCount; ++i )
        {
            ...
            {
              bl_tordbg( t, "IP:%s contributed to bad piece:%d. strike:%d",
                        tr_atomAddrStr( peer->atom ),
...
isPeerCandidate( const tr_torrent * tor, struct peer_atom * atom, const time_t now )
{
    ...
    {
        tr_tordbg( tor, "Blocklisted IP:%s dropped as atom peer", tr_address_to_string( &atom->addr ) ); 
        return FALSE;
    }
    ...
    if( atom->flags2 & MYFLAG_BANNED )
    {
	    /* tr_tordbg( tor, "Banned IP %s dropped as atom peer", tr_address_to_string( &atom->addr ) ); */
        return FALSE;
    }
    return TRUE;
}
